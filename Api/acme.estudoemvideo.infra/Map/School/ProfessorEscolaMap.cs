using acme.estudoemvideo.domain.DTO.Enum;
using acme.estudoemvideo.domain.DTO.School;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;
using System;
using System.Collections.Generic;
using System.Text;

namespace acme.estudoemvideo.infra.Map.School
{
    public class ProfessorEscolaMap : IEntityTypeConfiguration<ProfessorEscola>
    {
        public void Configure(EntityTypeBuilder<ProfessorEscola> builder)
        {
            builder.HasKey(t => t.Id);

            builder.ToTable("ProfessorEscola");
            builder.Property(t => t.DataCriacao).IsRequired(false);
            builder.Property(t => t.DataModificacao).IsRequired(false);
            builder.Property(t => t.DataInativacao).IsRequired(false);
            builder.Property(t => t.Status).HasDefaultValue(EnumStatus.Ativo);

            builder.Property(t => t.PopularidadeProfessor);
            builder.HasOne(t => t.Escola).WithMany(t => t.ProfessorEscolas).HasForeignKey(t => t.EscolaId);
            builder.HasOne(t => t.Usuario).WithMany(t => t.ProfessorEscolas).HasForeignKey(t => t.UsuarioId);

        }
    }
}
